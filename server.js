...
app.use((req, res, next) => {
    ...
    // index.htmlへのアクセスはそのまま許可（JS側で認証チェック）
    next();
});
        ...
        contracts: [
            {
                ...
                responseStatus: "対応中",
                preconfirmStatus: "前確待ち",
                postconfirmStatus: "後確待ち",
                handlingStatus: "対応中",
                preconfirmHistory: [],
                postconfirmHistory: [],
                handlingHistory: [],
...
app.post('/api/contracts/:id/history', (req, res) => {
    const contractId = req.params.id;
    const { content, responseStatus } = req.body;
    const historyData = readHistory();
    ...
    const newHistory = {
        ...
        content: content,
        responseStatus: responseStatus || '対応中',
        createdAt: createdAt
    };
...
// 前確履歴追加
app.post('/api/contracts/:id/preconfirm', requireAuth, (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // 前確履歴に追加
        if (!contract.preconfirmHistory) {
            contract.preconfirmHistory = [];
        }
        
        contract.preconfirmHistory.push({
        ...
        // ステータス更新
        contract.preconfirmStatus = status;
...
// 後確履歴追加
app.post('/api/contracts/:id/postconfirm', requireAuth, (req, res) => {
    const { id } = req.params;
    const { status, note, date, time } = req.body;
    ...
    try {
        ...
        // 後確履歴に追加
        if (!contract.postconfirmHistory) {
            contract.postconfirmHistory = [];
        }
        
        contract.postconfirmHistory.push({
            date: date || new Date().toISOString().split('T')[0],
            time: time || new Date().toTimeString().split(' ')[0].substring(0, 5),
            status,
            note: note || '',
        ...
        // ステータス更新
        contract.postconfirmStatus = status;
...
// 対応履歴追加（拡張版）
app.post('/api/contracts/:id/handling', requireAuth, (req, res) => {
    const { id } = req.params;
    const { status, content, date, time } = req.body;
    ...
    try {
        ...
        // 対応履歴に追加
        if (!contract.handlingHistory) {
            contract.handlingHistory = [];
        }
        
        contract.handlingHistory.push({
            date: date || new Date().toISOString().split('T')[0],
            time: time || new Date().toTimeString().split(' ')[0].substring(0, 5),
            status,
            content: content || '',
        ...
        // ステータス更新
        contract.handlingStatus = status;
        contract.responseStatus = status; // 既存の対応ステータスも更新
